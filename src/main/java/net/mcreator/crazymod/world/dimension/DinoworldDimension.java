...
package net.mcreator.crazymod.world.dimension;
...
import net.mcreator.crazymod.item.DinoworldItem;
import net.mcreator.crazymod.block.BlockofdecoratedfossilsBlock;
...
public class DinoworldDimension extends CrazyModModElements.ModElement {
	@ObjectHolder("crazy_mod:dinoworld_portal")
	public static final CustomPortalBlock portal = null;

	public DinoworldDimension(CrazyModModElements instance) {
		super(instance, 9);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("crazy_mod:dino_1")).getGenerationSettings().getSurfaceBuilder()
	...
	public void clientLoad(FMLClientSetupEvent event) {
		DimensionRenderInfo customEffect = new DimensionRenderInfo(Float.NaN, true, DimensionRenderInfo.FogType.NONE, false, false) {
			...
			public Vector3d func_230494_a_(Vector3d color, float sunHeight) {
				return new Vector3d(0.843137254902, 0.721568627451, 0.447058823529);
			}
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("crazy_mod:dinoworld"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("dinoworld_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("dinoworld_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("dinoworld_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new DinoworldItem().setRegistryName("dinoworld"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("crazy_mod:dinoworld"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("crazy_mod:dinoworld")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == BlockofdecoratedfossilsBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? BlockofdecoratedfossilsBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, BlockofdecoratedfossilsBlock.block.getDefaultState(), 3);
					}
				}
			}
